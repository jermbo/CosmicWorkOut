...
| **Missing save-error handling** — **Fixed in v1.11.0** ([US-050](../features/v1.11.0/US-050-cheap-hardening.md)). `handleCreate` / `handleSave` / `handleFinish` could strand `saving = true` with no feedback on DB failure.             | `CreateProgramSheet`, `WorkoutEditor`, `ExerciseFormSheet`, session overlays | The DB layer already toasts on write error; the UI-state reset is what's missing. Low effort. |
| **Streak/week uses UTC bucketing on local dates** — **Checked in v1.11.0: not a bug.** `isoWeekKey` builds the UTC date from the local calendar day; boundary tests now pin it ([US-050](../features/v1.11.0/US-050-cheap-hardening.md)). | `streak.ts`, `date.ts`                                                       | Verify with a boundary test before trusting long-streak displays.                             |
...
| Phase                   | Work                                                                                                                                                       | Trigger                                                        |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| **A. Cheap hardening**  | **Done in v1.11.0** ([US-050](../features/v1.11.0/US-050-cheap-hardening.md)) — backup file-size cap; save-error handling in sheets; streak boundary test. | Next time those files are touched.                             |
