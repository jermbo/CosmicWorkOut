...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Backup records written to IndexedDB with no per-record shape validation; a crafted backup can corrupt data or replace built-ins by importing seeded IDs with `isBuiltIn:false`.                                     | `backupPayload.ts` `assertStoresAreArrays` (store-level only), `database.ts` `upsertBuiltInRecords` | If backup import ever accepts files from untrusted sources, or we see corruption reports. Add per-store schema validation and/or a staging-DB-then-swap restore. |
| **Fixed in v1.10.0** — 50 MB cap before `file.text()` ([US-050](../features/v1.10.0/US-050-cheap-hardening.md)). Was: unbounded backup file read (`file.text()` before parse) — a multi-GB file can freeze the tab. | `settings/data/+page.svelte`                                                                        | Cheap to pre-empt: reject `file.size` over ~50 MB before parsing. Do it next time this file is touched.                                                          |
...
3. **Calendar per-cell scans** — _moot: the calendar was removed in v1.10.0 ([US-047](../features/v1.10.0/US-047-retire-history.md)); the week-strip half still applies._ — `O(habits × logs)` per cell (~30 cells) on every render;
...
| **Missing save-error handling** — **Fixed in v1.10.0** ([US-050](../features/v1.10.0/US-050-cheap-hardening.md)). `handleCreate` / `handleSave` / `handleFinish` could strand `saving = true` with no feedback on DB failure.             | `CreateProgramSheet`, `WorkoutEditor`, `ExerciseFormSheet`, session overlays | The DB layer already toasts on write error; the UI-state reset is what's missing. Low effort. |
| **Streak/week uses UTC bucketing on local dates** — **Checked in v1.10.0: not a bug.** `isoWeekKey` builds the UTC date from the local calendar day; boundary tests now pin it ([US-050](../features/v1.10.0/US-050-cheap-hardening.md)). | `streak.ts`, `date.ts`                                                       | Verify with a boundary test before trusting long-streak displays.                             |
...
| Phase                   | Work                                                                                                                                                       | Trigger                                                        |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| **A. Cheap hardening**  | **Done in v1.10.0** ([US-050](../features/v1.10.0/US-050-cheap-hardening.md)) — backup file-size cap; save-error handling in sheets; streak boundary test. | Next time those files are touched.                             |
