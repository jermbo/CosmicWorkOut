...
// ── Cat color map ────────────────────────────────────────────────
const CAT_COLOR = {
  Hinge: '#B2F042', Squat: '#B2F042', Push: '#B286FD',
  Pull: '#60C6FF', Lateral: '#E55733', Rotational: '#E55733',
  Power: '#B2F042', Carry: '#B286FD',
};
    ...
    <div className="scroll fade-in">
      <div className="scr-pad top-pad" style={{ paddingTop: 'max(22px, env(safe-area-inset-top))' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 28 }}>
          <div className="screen-title">Packet 1<span style={{ fontWeight: 500, color: 'var(--color-text-secondary)' }}> · Foundation</span></div>
        </div>

        {/* Packet progress bar */}
        <div style={{ marginBottom: 28 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 'var(--ls-wide)', textTransform: 'uppercase', color: 'var(--color-text-secondary)' }}>Week progress</span>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 13, color: 'var(--color-text-secondary)' }}>Wk 2 of 12</span>
          </div>
          <div style={{ height: 6, borderRadius: 4, background: 'var(--color-surface-3)', overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${(2/12)*100}%`, background: 'var(--accent)', borderRadius: 4, transition: 'width 600ms var(--ease-spring)' }} />
          </div>
        </div>
    ...
    <div className={`prog-card${isToday ? ' prog-card-today' : ''}`} style={{ '--waccent': accent }}>
      <div className="prog-card-head">
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--color-text-secondary)', marginTop: 2, fontWeight: 500 }}>{w.focus}</div>
        </div>
      ...
      <div className="prog-ex-chips">
        {w.exercises.map((ex, i) => (
          <span className="prog-ex-chip" key={i}>{ex.name}</span>
        ))}
      </div>
    </div>
  );
...
function WorkoutEditor({ workout: initWorkout, onBack }) {
  ...
  function addEx(libEx) {
    ...
    setWorkout(w => ({
      ...w,
      exercises: [...w.exercises, { id: libEx.id, name: libEx.name, sets: libEx.defaultSets, reps: libEx.defaultReps, unit: libEx.unit, _key: keyRef.current }],
    }));
    ...
    <div className="session enter">
      ...
      <div className="ses-top" style={{ paddingBottom: 14 }}>
        <div className="ses-bar">
          <button className="icon-btn" onClick={onBack} aria-label="Back"><Icon name="chevL" /></button>
          <div className="ses-titles" style={{ flex: 1 }}>
            {editTitle
              ? <input className="editor-title-input" autoFocus value={workout.title}
              ...
              : <div className="t" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  {workout.title}
                  <button style={{ background: 'none', border: 'none', color: 'var(--color-text-muted)', padding: 0, display: 'flex' }}
                    onClick={() => setEditTitle(true)} aria-label="Edit title">
                    <Icon name="edit" style={{ width: 15, height: 15 }} />
                  </button>
                </div>}
            <div className="s">Workout {workout.letter} · {workout.exercises.length} exercises</div>
          </div>
          <button className="prog-save-btn" onClick={onBack}>Save</button>
        </div>
      </div>
          ...
          {workout.exercises.map((ex, i) => (
            <div className="editor-row" key={ex._key ?? i}>
              ...
              <div className="editor-reorder">
                <button className="reorder-btn" onClick={() => moveEx(i, -1)} disabled={i === 0}><Icon name="chevL" style={{ transform: 'rotate(90deg)', width: 16, height: 16 }} /></button>
                <button className="reorder-btn" onClick={() => moveEx(i, 1)} disabled={i === workout.exercises.length - 1}><Icon name="chevL" style={{ transform: 'rotate(-90deg)', width: 16, height: 16 }} /></button>
              </div>
              {/* index */}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--color-text-secondary)', marginTop: 1 }}>
                  {(() => {
                    ...
                    return lib ? lib.muscles : ex.unit === 'lb' ? 'Weighted' : ex.unit === 'band' ? 'Band' : 'Bodyweight';
                  })()}
                </div>
              </div>
              {/* sets×reps tap to edit */}
              <button className="editor-sets-btn" onClick={() => setEditingEx(editingEx === i ? null : i)}>
                <span className="mono">{ex.sets}<span style={{ color: 'var(--color-text-muted)' }}>×</span>{ex.reps}</span>
      ...
      {/* Library sheet */}
      {showLibrary && (
        <ExerciseLibrary onAdd={addEx} onClose={() => setShowLibrary(false)} />
      )}
    </div>
  );
}
...
function InlineSetEditor({ ex, onSave, onCancel }) {
  ...
  return (
    <div className="inline-editor">
      <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: 'var(--ls-wide)', textTransform: 'uppercase', color: 'var(--color-text-secondary)', marginBottom: 12 }}>
        {ex.name}
      </div>
      <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
        ...
        <div className="ie-stepper">
          ...
          <div className="ie-row">
            <button className="step-btn" style={{ width: 38, height: 38, fontSize: 20 }} onClick={() => setSets(s => Math.max(1, s - 1))}><Icon name="minus" style={{ width: 18, height: 18 }} /></button>
            <span className="mono" style={{ fontSize: 26, fontWeight: 700, flex: 1, textAlign: 'center' }}>{sets}</span>
            <button className="step-btn" style={{ width: 38, height: 38, fontSize: 20 }} onClick={() => setSets(s => Math.min(8, s + 1))}><Icon name="plus" style={{ width: 18, height: 18 }} /></button>
          </div>
        </div>
        {/* Reps input */}
        <div className="ie-stepper">
          ...
          <div className="ie-row">
            <input className="ie-reps-input" value={reps} onChange={e => setReps(e.target.value)} placeholder="e.g. 10 ea" />
          </div>
        </div>
      </div>
      <div style={{ display: 'flex', gap: 8 }}>
        <button className="sheet-confirm" style={{ flex: 1 }} onClick={() => onSave(sets, reps)}><Icon name="check" />Done</button>
        <button className="icon-btn" style={{ width: 50, height: 50, flexShrink: 0, background: 'var(--color-surface-3)' }} onClick={onCancel}><Icon name="close" /></button>
      </div>
    </div>
  );
}
...
function ExerciseLibrary({ onAdd, onClose }) {
  ...
  const filtered = EXERCISE_LIBRARY.filter(ex => {
    ...
    const matchQ = !query || ex.name.toLowerCase().includes(query.toLowerCase()) || ex.muscles.toLowerCase().includes(query.toLowerCase());
    return matchCat && matchQ;
  });

  return (
    <div className="sheet-back" onClick={onClose}>
      <div className="sheet" style={{ maxHeight: '85dvh', display: 'flex', flexDirection: 'column', padding: '10px 0 0' }} onClick={e => e.stopPropagation()}>
        <div className="sheet-grab" style={{ margin: '0 auto 14px' }} />

        {/* Header */}
        <div style={{ padding: '0 20px 12px', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 20, letterSpacing: 'var(--ls-tight)' }}>Exercise Library</span>
            <button className="icon-btn" onClick={onClose}><Icon name="close" /></button>
          </div>
          {/* Search */}
          <div className="lib-search">
            <Icon name="dumbbell" style={{ width: 15, height: 15, color: 'var(--color-text-muted)', flexShrink: 0 }} />
            <input placeholder="Search exercises or muscles…" value={query} onChange={e => setQuery(e.target.value)} />
          </div>
        </div>
        ...
        <div className="lib-cats" style={{ flexShrink: 0 }}>
          {LIBRARY_CATS.map(c => (
            <button key={c} className={`lib-cat${cat === c ? ' active' : ''}`} onClick={() => setCat(c)}>{c}</button>
          ))}
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 20px calc(16px + env(safe-area-inset-bottom))' }}>
          {filtered.length === 0 && (
            <div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--color-text-muted)', fontSize: 13 }}>No exercises match.</div>
          )}
  ...
  return (
    <div className={`lib-row${open ? ' lib-row-open' : ''}`}>
      <div className="lib-row-main" onClick={() => setOpen(o => !o)}>
        ...
        <span className="lib-cat-tag" style={{ '--dt': dot }}>{ex.cat}</span>
        <span className="mono" style={{ fontSize: 12, color: 'var(--color-text-secondary)', flexShrink: 0 }}>{ex.defaultSets}×{ex.defaultReps}</span>
        <Icon name={open ? 'chevL' : 'chevR'} style={{ width: 15, height: 15, color: 'var(--color-text-muted)', transform: open ? 'rotate(90deg)' : 'rotate(-90deg)', flexShrink: 0 }} />
      </div>
      {open && (
