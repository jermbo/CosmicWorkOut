    ...
    <div className="scroll fade-in">
      <div className="scr-pad top-pad" style={{ paddingTop: 'max(22px, env(safe-area-inset-top))' }}>
        <div className="today-head">
          <div>
            ...
            <div className="screen-title" style={{ marginTop: 6 }}>Today</div>
          </div>
          <div className="streak-pill">
            <Icon name="flame" style={{ width: 15, height: 15, color: 'var(--accent)' }} />
            <span><b>5</b> wk streak</span>
          </div>
        </div>
      </div>
      ...
      <div className="pkt-card">
        <div className="pkt-tabs">
          <span className="pkt-tab">
            <svg className="staple" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round"><path d="M5 8v8M9 6v10M13 8v8M17 6v10" /></svg>
        ...
        <div className="pkt-meta">
          {w.focus.split(' · ').map((f, i) => (
            <span className="focus-chip" key={i}><span className="dot" />{f}</span>
          ))}
          <span className="focus-chip"><Icon name="timer" style={{ width: 13, height: 13 }} />~{w.estMin} min</span>
        </div>
        ...
        <div className="pkt-exlist">
          {w.exercises.map((ex, i) => (
            <div className="pkt-exrow" key={ex.id}>
              ...
              <span className="tg">{ex.sets}×{ex.reps}</span>
            </div>
          ))}
        </div>
      ...
      <div className="scr-pad">
        <div className="section-label">
          <h3>This week</h3>
          <span className="count">{weekDone}/{weekTotal} done</span>
        </div>
