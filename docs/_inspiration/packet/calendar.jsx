      ...
      <div className="scr-pad top-pad" style={{ paddingTop: 'max(22px, env(safe-area-inset-top))' }}>
        ...
        <div className="cal-head">
          ...
          <div className="cal-nav">
            <button className="icon-btn" aria-label="Previous month"><Icon name="chevL" /></button>
            <button className="icon-btn" aria-label="Next month"><Icon name="chevR" /></button>
          </div>
        </div>
        ...
        <div className="cal-stats">
          <div className="stat-box">
            <div className="v accent">{doneDays.length}<small> / {doneDays.length + scheduledCount}</small></div>
            <div className="l">Sessions done</div>
          </div>
          <div className="stat-box">
            <div className="v">{(totalVolume / 1000).toFixed(1)}<small>k lb</small></div>
            <div className="l">Volume lifted</div>
          </div>
          <div className="stat-box">
            <div className="v">5<small> wk</small></div>
        ...
        <div className="cal-legend">
          <span className="leg"><span className="sw done" />Completed</span>
          <span className="leg"><span className="sw scheduled" />Scheduled</span>
          <span className="leg"><span className="sw skipped" />Skipped</span>
          <span className="leg"><span className="sw rest" />Rest</span>
        </div>
      </div>
      ...
      <div className="sheet" onClick={e => e.stopPropagation()}>
        ...
        <div className="day-sum-head">
          ...
          <div className="day-sum-titles">
            <div className="t">Workout {info.workout} · {info.title}</div>
            <div className="s">Week {info.week} · completed</div>
          </div>
        </div>

        <div className="day-stats">
          <div className="day-stat"><div className="v">{info.min}m</div><div className="l">Duration</div></div>
          <div className="day-stat"><div className="v">{log ? log.length : info.sets}</div><div className="l">Exercises</div></div>
          <div className="day-stat"><div className="v">{(info.volume / 1000).toFixed(1)}k</div><div className="l">lb volume</div></div>
        </div>
