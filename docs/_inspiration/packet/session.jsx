...
function SessionView({ workout: w, loggingMode, completionLevel, showEmoji, accent, onClose, onFinish }) {
  ...
  function commitSet(exIdx, setIdx, value) {
    ...
    setLastUsed(prev => { const n = prev.slice(); n[exIdx] = value.weight; return n; });
  }
    ...
    <div className="session enter">
      ...
      <div className="ses-top">
        <div className="ses-bar">
          <button className="icon-btn" onClick={onClose} aria-label="Back"><Icon name="chevL" /></button>
          <div className="ses-titles">
            <div className="t">Workout {w.workout} · {w.title}</div>
            <div className="s">Packet {w.packet} · Week {w.week}</div>
          </div>
          <div className="ses-timer"><span className="lbl">Elapsed</span><span className="mono">{mmss}</span></div>
        </div>
        <div className="ses-progress"><i style={{ width: `${(doneSets / totalSets) * 100}%` }} /></div>
      </div>
      ...
      <div className="scroll" ref={listRef}>
        <div className="ses-list">
              ...
              <div key={ex.id}
                ...
                <div className="ex-head">
                  ...
                  <div className="ex-target">
                    <div className="v">{ex.sets}×{ex.reps}</div>
          ...
          <div style={{ textAlign: 'center', color: 'var(--color-text-muted)', fontSize: 12, padding: '14px 0 4px', letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 700 }}>
      ...
      <div className="finish-bar">
        <button className={`finish-btn${allDone ? '' : ' muted'}`}
          onClick={() => onFinish({ doneSets, totalSets, elapsed, exDoneCount: exDone.filter(Boolean).length })}>
          {allDone ? <><Icon name="flag" style={{ width: 18, height: 18 }} /> Finish session</> : `Finish early · ${doneSets}/${totalSets} sets`}
        </button>
      </div>
      ...
      {sheet && (
        <LogSheet
          ...
          onSave={(value) => { commitSet(sheet.exIdx, sheet.setIdx, value); setSheet(null); }}
        />
      )}
    </div>
  );
...
function LogSheet({ ex, setIdx, mode, existing, prefillWeight, onClose, onSave }) {
  ...
  // number pad
  function padPress(k) {
    if (k === 'del') { setPadStr(s => s.slice(0, -1)); return; }
    if (k === 'next') { applyPad(); setPadField(f => (f === 'weight' ? 'reps' : 'weight')); setPadStr(''); return; }
  ...
  function applyPad() {
    ...
    if (padField === 'weight') setWeight(v); else setReps(v);
  }
  ...
  function save() {
    ...
    const finalReps = mode === 'pad' && padField === 'reps' && padStr !== '' ? parseInt(padStr, 10)
                    : mode === 'pad' && padField === 'weight' && padStr !== '' ? reps : reps;
    ...
    <div className="sheet-back" onClick={onClose}>
      <div className="sheet" onClick={e => e.stopPropagation()}>
        ...
        <div className="sheet-prev">
          {ex.last != null
            ? <>Last time · <b>{isBand ? ex.last : `${ex.last}${isLb ? ' lb' : ''}`}</b> × {defReps}{suffix ? ' ' + suffix : ''}</>
            : <>Target · <b>{defReps}{suffix ? ' ' + suffix : ''}</b> · bodyweight</>}
        </div>
        ...
        {mode === 'steppers' ? (
          <div className="stepper-grp">
            {!isBw && (
              <div className="stepper">
                ...
                <div className="row">
                  <button className="step-btn" onClick={() => bumpWeight(-1)}><Icon name="minus" style={{ width: 22, height: 22 }} /></button>
                  <div className="step-val">
                    <div className="n">{isBand ? weight : weight}</div>
                    <div className="u">{isBand ? 'level' : 'lb'}</div>
                  </div>
                  <button className="step-btn" onClick={() => bumpWeight(1)}><Icon name="plus" style={{ width: 22, height: 22 }} /></button>
                </div>
              </div>
            )}
            <div className="stepper">
              ...
              <div className="row">
                <button className="step-btn" onClick={() => setReps(r => Math.max(0, r - (suffix === 's' ? 5 : 1)))}><Icon name="minus" style={{ width: 22, height: 22 }} /></button>
                <div className="step-val">
                  <div className="n">{reps}</div>
                  <div className="u">{suffix === 's' ? 'sec' : suffix === 'ea' ? 'each' : 'reps'}</div>
                </div>
                <button className="step-btn" onClick={() => setReps(r => r + (suffix === 's' ? 5 : 1))}><Icon name="plus" style={{ width: 22, height: 22 }} /></button>
              </div>
            </div>
          </div>
        ) : (
          <>
            <div className="pad-field">
              {!isBw && (
                <div className={`pad-slot${padField === 'weight' ? ' active' : ''}`} onClick={() => { applyPad(); setPadField('weight'); setPadStr(''); }}>
                  <div className="l">{isBand ? 'Band' : 'Weight (lb)'}</div>
                  <div className="v">{padWeight}</div>
                </div>
              )}
              <div className={`pad-slot${padField === 'reps' ? ' active' : ''}`} onClick={() => { applyPad(); setPadField('reps'); setPadStr(''); }}>
            ...
            <div className="pad">
              ...
              <button className="pad-key fn" onClick={() => padPress('del')}>Del</button>
              <button className="pad-key" onClick={() => padPress('0')}>0</button>
              <button className="pad-key fn" onClick={() => padPress('next')}>Next</button>
            </div>
          </>
        )}
