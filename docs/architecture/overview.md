...
The shipped routes: **Overview** (`/`), **Habits** (`/habits`), **Workout** — plans list, today's routine, new plan, plan detail (`/workout`, `/workout/today`, `/workout/new`, `/workout/plan/[id]`; a plan's optional goal lives on the plan detail page, not a separate route), **Activity Log** (`/log`), **Insights** (`/insights`), **Health** (`/health`), **Baselines** (`/baselines`, `/settings/baselines`), and **Settings** (`/settings`) with `habits` / `workout` / `data` sub-routes — plus global overlays (active session, completion screen, crash recovery) in the root layout. Belly Dance and the standalone Practice hub / Lift plans area were removed in v1.10.0 ([US-051](../features/v1.10.0/US-051-remove-belly-dance.md), [US-052](../features/v1.10.0/US-052-one-workout-section.md)). There is no `/settings/appearance` route; appearance lives on `/settings/personalization`.
